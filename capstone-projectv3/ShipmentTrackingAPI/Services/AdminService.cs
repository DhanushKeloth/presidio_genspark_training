...
public class AdminService : IAdminService
{
    ...
    {
        var pending   = await _ctx.Shipments.CountAsync(s => s.Status == ShipmentStatus.Pending);
        var assigned  = await _ctx.Shipments.CountAsync(s => s.Status == ShipmentStatus.Assigned);
        var pickedUp  = await _ctx.Shipments.CountAsync(s => s.Status == ShipmentStatus.PickedUp);
        var inTransit = await _ctx.Shipments.CountAsync(s => s.Status == ShipmentStatus.InTransit);
        var arrived   = await _ctx.Shipments.CountAsync(s => s.Status == ShipmentStatus.Arrived);
