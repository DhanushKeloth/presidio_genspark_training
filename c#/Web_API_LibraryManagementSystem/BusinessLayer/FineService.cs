...
namespace LibraryManagementSystem.Services
{
    public class FineService : IFineService
    {
        ...
        public bool ProcessFinePayment(int memberId, decimal amount)
        {
            ...
            // Calculate current pending balance dynamically
            decimal currentBalance = _fineRepo.GetTotalUnpaidFine(memberId);

            if (currentBalance == 0)
            {
                throw new BusinessRuleException("ℹ️ Account Clear: This member currently has no outstanding fine balances.");
            }
            ...
            using (var transaction = _borrowingRepo.BeginTransaction())
            {
                try
                {
                    var unpaidFines = _fineRepo.GetHistoryByMemberId(memberId)
                        .Where(f => !f.IsPaid)
                        .OrderBy(f => f.PaymentDate) // Pay off the oldest fines first
                        .ToList();
                    ...
                    foreach (var fine in unpaidFines)
                    {
                        ...
                        // If the payment completely clears or exceeds this specific fine row
                        if (remainingPaymentToApply >= fine.Amount)
                        {
                            remainingPaymentToApply -= fine.Amount;
                            fine.IsPaid = true; // Mark this row completely cleared!
                            _fineRepo.UpdateMember(fine);
                        }
                        else
                        {
                            // Partial payment handling if your schema supports splitting 
                            // Otherwise, we mark it fully paid once the payment chunk satisfies the block
