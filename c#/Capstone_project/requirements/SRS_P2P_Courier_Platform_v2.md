...
**Version:** 2.0 | **Status:** Approved for Development | **Date:** May 2026
...
| ORM | Entity Framework Core Code-First | EF Core 8 |
| Database | PostgreSQL | 15+ |
| Real-Time | ASP.NET Core SignalR | Built-in .NET 8 |
| Authentication | JWT Bearer + Refresh Tokens | Microsoft.AspNetCore.Authentication.JwtBearer |
...
The Sender and Recipient may be the same person (self-delivery) or different people. The system distinguishes them via recipient_name, recipient_phone, and dropoff_address fields on the Shipment.
...
| Login | POST | /api/auth/login | Validates credentials. Returns access JWT + refresh token. |
| Refresh token | POST | /api/auth/refresh | Rotates refresh token. Returns new access JWT. |
| Logout | POST | /api/auth/logout | Revokes refresh token in DB. |
| Book shipment | POST | /api/shipments | Creates Shipment + ShipmentItems rows. Generates TrackingNumber. Status = Pending. |
...
| FR0.4 | The system shall issue a refresh token alongside the access token. The refresh token shall be stored as a SHA-256 hash in the RefreshTokens table with a configurable expiry defaulting to 7 days. | Must Have |
...
| FR0.7 | The refresh token endpoint shall validate the submitted token against the hashed DB record, verify it is not revoked and not expired, issue a new access JWT, rotate the refresh token by revoking the old one and issuing a new one, and return the new token pair. | Must Have |
| FR0.8 | The logout endpoint shall mark the current refresh token as revoked (is_revoked = TRUE) in the database, ending the session immediately. | Must Have |
...
| FR1.6 | Customers shall retrieve full detail of any shipment they own, including all ShipmentItems and the complete ShipmentEvents timeline ordered by occurred_at ascending. | Must Have |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR3.1 | Once a Driver has self-assigned a shipment with status=Assigned, they shall trigger a Request Pickup OTP action. The backend shall generate a cryptographically random 4-digit numeric OTP, store it in pickup_otp_code with pickup_otp_expires_at = UTC now + 15 minutes, and broadcast it to the Sender's live tracking session via SignalR. | Must Have |
...
| FR3.3 | The Driver shall enter the OTP verbally obtained from the Sender. The backend shall validate: OTP matches, not expired, attempt count 3 or under. On success: status = PickedUp, picked_up_at = UTC now, OTP fields cleared, ShipmentEvent inserted, SignalR broadcast sent. | Must Have |
| FR3.4 | On each failed OTP attempt, pickup_otp_attempt_count shall increment. At 3 failed attempts, the endpoint shall return HTTP 429 with message "Maximum attempts reached. Please regenerate the OTP." | Must Have |
| FR3.5 | If the Pickup OTP window expires, the Driver shall call the regenerate endpoint. A new OTP shall be generated, the expiry reset to UTC now + 15 minutes, and the attempt count reset to 0. The new OTP shall be pushed to the Sender via SignalR. | Must Have |
...
| FR5.2 | The Driver shall then trigger Request Delivery OTP. The backend shall generate a cryptographically random 4-digit numeric OTP, store it in delivery_otp_code with delivery_otp_expires_at = UTC now + 15 minutes, and broadcast it to the Recipient's live tracking session via SignalR. | Must Have |
| FR5.3 | The Recipient's tracking screen shall display the Delivery OTP with a visible countdown timer. | Must Have |
| FR5.4 | The Driver shall enter the Delivery OTP verbally obtained from the Recipient. The backend shall validate: OTP matches, not expired, attempt count 3 or under. On success: status = Delivered, delivered_at = UTC now, OTP fields cleared, ShipmentEvent inserted. | Must Have |
...
| FR5.6 | On each failed Delivery OTP attempt, delivery_otp_attempt_count shall increment. At 3 failures the API shall return HTTP 429 requiring regeneration before further attempts. | Must Have |
| FR5.7 | If the Delivery OTP window expires, the Driver shall call the regenerate endpoint. A new OTP is generated, expiry reset, attempt count reset to 0, and new OTP pushed to Recipient via SignalR. | Must Have |
...
| FR6.2 | Admins shall update a Driver's account_status to any valid value. The approved_at timestamp shall be recorded when transitioning to Active. | Must Have |
...
- Refresh tokens shall be stored as SHA-256 hashes, never plain text.
- JWT signing secret shall be stored in environment variables or appsettings.json secrets — never hardcoded.
- OTP submission shall be rate-limited to 3 attempts per window enforced via otp_attempt_count in the database.
...
- Database indexes shall be created on: shipments.tracking_number (unique), shipments.customer_id, shipments.driver_id, shipments.status, shipment_events.shipment_id, users.email (unique).
...
Users ──1:N──► RefreshTokens          (user_id FK)
Shipments ──1:N──► ShipmentItems      (ON DELETE CASCADE)
Shipments ──1:N──► ShipmentEvents     (ON DELETE CASCADE)
```
...
| password_hash | TEXT | NOT NULL | ASP.NET Identity hasher output |
| role | ENUM | NOT NULL | Customer, Driver, Admin |
| is_active | BOOLEAN | NOT NULL, DEFAULT TRUE | Soft-delete flag |
| created_at | TIMESTAMPTZ | NOT NULL | UTC |
| updated_at | TIMESTAMPTZ | NOT NULL | Auto-updated on any write |
...
| op_status | ENUM | NULLABLE | Available, InTransit, Offline |
...
| approved_at | TIMESTAMPTZ | NULLABLE | Set when Admin approves |
| created_at | TIMESTAMPTZ | NOT NULL | UTC |
...
| expires_at | TIMESTAMPTZ | NOT NULL | Default: issued + 7 days |
| is_revoked | BOOLEAN | NOT NULL, DEFAULT FALSE | |
| device_hint | VARCHAR(100) | NULLABLE | Browser or device label |
| created_at | TIMESTAMPTZ | NOT NULL | UTC |
...
|-------|------|-------------|-------|
| id | INT | PK, auto-increment | |
| tracking_number | VARCHAR(20) | NOT NULL, UNIQUE | TRK-XXXXXX |
| customer_id | INT | FK Users, NOT NULL | Sender — always set |
| driver_id | INT | FK Users, NULLABLE | NULL until assigned; ON DELETE SET NULL |
| pickup_address | TEXT | NOT NULL | |
| pickup_lat | DOUBLE | NULLABLE | Geocoded at booking |
| pickup_lng | DOUBLE | NULLABLE | Geocoded at booking |
| dropoff_address | TEXT | NOT NULL | |
| dropoff_lat | DOUBLE | NULLABLE | Geocoded at booking |
| dropoff_lng | DOUBLE | NULLABLE | Geocoded at booking |
| recipient_name | VARCHAR(100) | NOT NULL | |
| recipient_phone | VARCHAR(20) | NOT NULL | |
| status | ENUM | NOT NULL, DEFAULT Pending | See state machine |
| pickup_otp_code | CHAR(4) | NULLABLE | Cleared after PickedUp confirmed |
| pickup_otp_expires_at | TIMESTAMPTZ | NULLABLE | 15-minute window |
| pickup_otp_attempt_count | SMALLINT | NOT NULL, DEFAULT 0 | Max 3 |
| delivery_otp_code | CHAR(4) | NULLABLE | Cleared after Delivered confirmed |
| delivery_otp_expires_at | TIMESTAMPTZ | NULLABLE | 15-minute window |
| delivery_otp_attempt_count | SMALLINT | NOT NULL, DEFAULT 0 | Max 3 |
| picked_up_at | TIMESTAMPTZ | NULLABLE | UTC — set on PickedUp |
| delivered_at | TIMESTAMPTZ | NULLABLE | UTC — set on Delivered |
| created_at | TIMESTAMPTZ | NOT NULL | UTC |
| updated_at | TIMESTAMPTZ | NOT NULL | Auto-updated |

Indexes: idx_shipments_tracking (unique), idx_shipments_customer_id, idx_shipments_driver_id, idx_shipments_status
...
Computed total shipment weight = SUM(weight_kg x quantity) across all items for a shipment.
...
| actor_id | INT | NULLABLE, FK Users | Who triggered the event; NULL = system |
...
Index: idx_events_shipment_id

### 8.8 ShipmentStatus Enum and State Machine
...
| POST | /api/auth/login | email, password | 200 with accessToken, refreshToken, expiresIn |
| POST | /api/auth/refresh | refreshToken | 200 with accessToken, refreshToken, expiresIn |
...
| GET | /api/shipments/{id} | Full shipment detail with items and events timeline |
...
**Step 1 — Booking**
Customer submits booking form. API creates Shipment + ShipmentItems. TrackingNumber generated. Status = Pending. ShipmentEvent inserted: "Shipment booked".
...
**Step 3 — Pickup OTP**
Driver calls request-pickup-otp. Backend generates 4-digit OTP. Stored in pickup_otp_code with 15-min expiry. SignalR pushes PickupOtpGenerated event to Sender's connection only. Sender's screen shows OTP and countdown. Sender reads OTP aloud to Driver. Driver enters OTP in Driver UI. Backend validates: match, not expired, attempts under 3. Status = PickedUp. picked_up_at = UTC now. OTP fields cleared. ShipmentEvent: "Parcel collected from sender — POP confirmed". SignalR broadcasts StatusUpdated to group.
...
Background Service fires every 5 seconds. Queries all InTransit shipments. For each: interpolates Driver's coordinates toward dropoff. Updates DriverProfile.current_lat and current_lng. Calls IHubContext to broadcast LocationUpdated to shipment's SignalR group. Sender and Recipient tracking screens update map pin in real time without page refresh.
...
| Any | Any | Admin override | Admin role required; creates override ShipmentEvent |
...
Initialise .NET 8 Web API with folder structure: Controllers, Services, Repositories, Models, DTOs, Hubs, BackgroundServices, Middleware, Enums. Initialise Angular project with feature modules: auth, customer, driver, admin, shared. Configure PostgreSQL connection string. Configure AppDbContext with all entity DbSets. Configure global exception handler middleware. Configure CORS policy for localhost:4200.
Checkpoint: API boots and returns 200 on health endpoint. Angular loads blank modules.

### Phase 1 — Authentication and JWT (Days 2–3)
Define User, DriverProfile, RefreshToken entities. Create and apply EF Core migration. Implement AuthService: register, login with JWT issuance, refresh token rotation, logout. Configure JWT Bearer middleware. Build AuthController. Build Angular AuthModule with register and login forms. Implement AuthGuard and AuthInterceptor in Angular. Store access token in Angular service memory; refresh token in HttpOnly cookie.
Checkpoint: Register Customer and Driver, login both, inspect JWT claims, confirm 401 on protected endpoint.

### Phase 2 — Shipment Booking (Days 4–5)
Define Shipment, ShipmentItem, ShipmentEvent entities and all status enums. Create and apply migration. Implement TrackingNumber generator. Implement ShipmentService: create, list paginated, get by ID, public tracking lookup. Build ShipmentController. Build Angular CustomerModule with booking form, shipment list, and static tracking detail page.
Checkpoint: Book shipment, retrieve paginated list, look up by TrackingNumber.
...
Implement cryptographic OTP generator. Implement ShipmentService methods: request-pickup-otp, verify-pickup-otp, request-delivery-otp, verify-delivery-otp, regenerate-otp. Wire all OTP endpoints. Build Angular Driver UI: OTP input prompts for both pickup and delivery. Build Angular Customer/Recipient UI: OTP display cards with countdown timers.
Checkpoint: Full dual-OTP flow. Test expiry, wrong code, 3-attempt lockout, and regeneration for both pickup and delivery.

### Phase 5 — SignalR Real-Time (Days 10–11)
Define TrackingHub with JoinShipmentGroup and LeaveShipmentGroup. Implement GPS Background Service: query InTransit shipments, interpolate coordinates, push LocationUpdated via IHubContext. Wire OTP generation methods to push targeted events to Sender or Recipient connection only. Wire all status transitions to broadcast StatusUpdated, DriverArrived, and ShipmentDelivered. Build Angular SignalRService wrapping the @microsoft/signalr HubConnection. Connect tracking page to SignalR. Implement reconnection logic.
...
| OTP brute-force (4-digit = 10,000 combinations) | Low | High | Max 3 attempts enforced in DB via attempt_count; lockout requires regeneration |
...
| JWT stored in localStorage exposing to XSS | Medium | High | Store access token in Angular memory service; refresh token in HttpOnly cookie |
| EF Core migration conflicts in team development | Medium | Medium | Never edit existing migrations; always add new ones |
...
*End of Document*
*SRS Version 2.0 and Requirements Understanding Document*
